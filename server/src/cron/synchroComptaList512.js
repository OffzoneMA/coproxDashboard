...
let FinalContrat = [];
const boardId = 5087860221;
const typeData="synchroComptaList512"

const synchroMandats = {
    start: async () => {
        console.log('Start Extraction ...');
        logs.logExecution("synchroComptaList512")
        let counterStart =await vilogiService.countConenction();
        
        //const LogId = await scriptService.logScriptStart('synchroComptaList512');
        ...
        try {
            ...
            // Array of account codes to process
            const accountCodes = [
                { code: "51200001", name: "512" },
                { code: "50200001", name: "502" }
            ];
            ...
            for (const copro of copros) {
                ...
                if (copro.idVilogi !== undefined) {
                    ...
                    // Loop through each account code
                    for (const account of accountCodes) {
                        let budgetj30 = await vilogiService.getbudgetComptebyDate(copro.idVilogi, account.code, thirtyDaysAgo);
                        let budgetj = await vilogiService.getbudgetComptebyDate(copro.idVilogi, account.code, datedujour);
                        console.log(`Account ${account.name} - J-30:`, budgetj30);
                        console.log(`Account ${account.name} - Today:`, budgetj);
                        ...
                        const itemName = `Compte ${account.name} - ${copro.idCopro}`;
        ...
        let VolumeCalls = counterEnd[0].nombreAppel - counterStart[0].nombreAppel           
            //await scriptService.updateLogStatus('synchroComptaList512',LogId ,0 ,`Script executed successfully `, VolumeCalls );
            
            console.log('--------------------------------------------------------------------------------------------END Extraction ...');
        } catch (error) {
            let counterEnd =await vilogiService.countConenction(); 
            let VolumeCalls = counterEnd[0].nombreAppel - counterStart[0].nombreAppel           
            await scriptService.updateLogStatus('synchroComptaList512',LogId ,-1,`An error occurred: ${error.message} `, VolumeCalls );
